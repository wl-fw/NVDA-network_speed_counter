...
try:
    ...
    class _Py3Utf8Output(TextIOWrapper):
        ...
        def __init__(self, f, **kwargs):
            if hasattr(f, 'buffer') and f.buffer is not None:
                buf_target = f.buffer
            else:
                raise TypeError(
                    "Não é possível envolver o objeto '%s' (%s) com TextIOWrapper: "
                    "ele não possui a interface de buffer esperada (.buffer ou métodos de fluxo brutos)." %
