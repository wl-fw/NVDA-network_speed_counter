...
# Tradutores: Título do painel de configurações, Rótulo da caixa de seleção para mostrar data, download, upload, ping, servidor, ID do servidor, IP do servidor, patrocinador do servidor, localização do servidor, distância do servidor, URL do servidor, IP do cliente, ISP do cliente, localização do cliente, bytes enviados, bytes recebidos, duração do teste, link de compartilhamento, threads de download, threads de upload, tamanhos de download, tamanhos de upload, exibir diálogo de erros, mensagem de erro quando nenhuma opção de exibição está selecionada
